...
- Use ids q1, q2, q3, etc. Preserve explicit source identifiers such as Q1 as labels; otherwise use labels Q1, Q2, Q3, etc.
...
export default function (pi: ExtensionAPI) {
  pi.registerCommand("answer", {
    description:
      "Extract questions from last assistant message and answer them interactively",
    handler: async (_args, ctx) => {
      ...
      const result = await ctx.ui.custom<string | null>(
        (tui, theme, _kb, done) => {
          const loader = new BorderedLoader(
            tui,
            theme,
            `Extracting questions using ${extractionModel.id}:${EXTRACTION_REASONING}...`,
          );
      ...
      const questionnaireResult = await runQuestionnaire(pi, ctx, {
        questions,
      });
      ...
      const answerById = new Map(
        questionnaireResult.details.answers.map((answer) => [
          answer.id,
          answer.label,
        ]),
      );
