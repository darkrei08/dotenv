...
export function registerTools(pi: ExtensionAPI, runtime: Runtime): void {
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      // Apply the SAME char limit + 40 KB JSON byte cap that publicItem uses, then
      // derive next_offset_chars from what is ACTUALLY returned. Slicing by the
      // requested char count while capping by bytes made the advance skip the
      // bytes the cap dropped, so paginating lost data (TODO.md P0 #2).
      const capped = truncate(full.slice(offset, offset + requested).join(""), requested);
      const returnedChars = chars(capped.text).length;
      const nextOffset = offset + returnedChars;
      const value = {
        ...publicItem({ ...item, content: capped.text }, requested),
        content_truncated: capped.truncated || nextOffset < full.length,
        offset_chars: offset,
